...
#define ROTH_WLAN_PWR	TEGRA_GPIO_PK4
#define ROTH_WLAN_RST	TEGRA_GPIO_PK2
#define ROTH_WLAN_WOW	TEGRA_GPIO_PU5
#define ROTH_SD_CD	TEGRA_GPIO_INVALID
#define WLAN_PWR_STR	"wlan_power"
#define WLAN_WOW_STR	"sd8xxx_sdmmc"

#define ROTH_BT_HOST_WAKE	TEGRA_GPIO_PU6
...
static struct mbtc_platform_data roth_mbtc_control = {
...
static struct platform_device roth_bt_device = {
	.name		= "mrvl_bt",
	.id		= 1,
	.num_resources	= ARRAY_SIZE(bt_resource),
	.resource	= bt_resource,
	.dev		= {
		.platform_data = &roth_mbtc_control,
	},
};
...
int __init roth_sdhci_init(void)
{
	...
	bt_resource[0].start = bt_resource[0].end =
		gpio_to_irq(ROTH_BT_HOST_WAKE);


	platform_device_register(&roth_wifi_device);
	platform_device_register(&roth_bt_device);
	return 0;
}
